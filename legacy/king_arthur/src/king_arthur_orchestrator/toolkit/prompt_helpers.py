"""
Prompt template helpers for King Arthur's agent system.

Provides standardized prompt construction with schema hints, constraint
enforcement, and best practices for JSON generation.

Part of the JSON fix plan (`docs/RESEARCH.md`).
"""

import json
from typing import Optional, Any, Dict
from king_arthur_orchestrator.json_engine.contracts import JsonContract
...
def json_context(schema_id: str, include_full_schema: bool = False, include_skeleton: bool = True) -> str:
    ...
    Returns:
        Formatted prompt text to append to knight prompts

    Example:
        system_prompt = "You are a task_knight." + json_context("execution_plan")
    """
    try:
    ...
    lines.append(f"Expected Schema: {contract.schema.get('title', schema_id)}")
    ...
    if required:
        ...
        for field in required:
            desc = contract.get_field_description(field)
            field_spec = contract.schema.get('properties', {}).get(field, {})
            field_type = field_spec.get('type', 'any')
    ...
    # Optional fields
    all_fields = set(contract.schema.get('properties', {}).keys())
    ...
    # Enum constraints
    for field_name, field_spec in contract.schema.get('properties', {}).items():
        if 'enum' in field_spec:
    ...
    if include_skeleton:
        ...
        lines.append("```json")
        skeleton = _generate_json_skeleton(contract.schema, include_descriptions=True)
    ...
    # Optionally include JSON skeleton example
    if include_skeleton:
        try:
            skeleton = _generate_json_skeleton(contract.schema, include_descriptions=True)
            lines.append("Example JSON Structure:")
            lines.append("```json")
            lines.append(json.dumps(skeleton, indent=2))
            lines.append("```")
            lines.append("")
        except Exception:
            # If skeleton generation fails, skip it
            pass
