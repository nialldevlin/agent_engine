"""Smart File Context Extraction - Intelligent workspace file inclusion.

Stage 4 Implementation: Auto-include relevant files based on query and conversation.

Based on research from `RESEARCH.md` §3.3:
...
- Semantic similarity to query (if embeddings available)
...
from king_arthur_orchestrator.infra.config import ArthurConfig, ArthurCostMode
from king_arthur_orchestrator.memory.embeddings import EmbeddingBackend, NullBackend, cosine_similarity
from king_arthur_orchestrator.toolkit.text_analysis import extract_keywords
...
class FileContextExtractor:
    ...
    Stage 4: Automatically includes relevant workspace files based on:
    - Semantic similarity to query (if embeddings available)
    - File mentions in recent conversation
    - File modification recency
    - File type and extension
    ...
    def __init__(
        self,
        workspace_root: Path,
        config: Optional[ArthurConfig] = None,
        ...
        Args:
            ...
            config: Arthur configuration (for mode-aware thresholds)
            embedding_backend: Optional embedding backend for semantic similarity
        """
        self.workspace_root = workspace_root
        self.config = config or ArthurConfig()
        self.embedding_backend = embedding_backend or NullBackend()
        self.has_embeddings = (
            self.embedding_backend.is_available() and
            self.embedding_backend.embedding_dim() > 0
        )

        # Get thresholds for current mode
        self.thresholds = FILE_SIZE_THRESHOLDS[self.config.cost_mode]
