...
def test_agent_runtime_validates_output_schema() -> None:
    ...
    context = ContextAssembler(ContextStore()).build_context(spec, ContextRequest(context_request_id="c1", budget_tokens=0, domains=[], history_types=[]))
    output, err = runtime.run_agent_stage(spec, stage, context)
    assert err is None
...
def test_tool_runtime_security_gate() -> None:
    ...
    context = ContextAssembler(ContextStore()).build_context(spec, ContextRequest(context_request_id="c1", budget_tokens=0, domains=[], history_types=[]))
    output, err = runtime.run_tool_stage(spec, stage, context)
    assert output is None
    assert err is not None
