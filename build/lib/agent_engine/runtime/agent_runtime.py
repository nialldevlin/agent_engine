...
class AgentRuntime:
    ...
    def _get_or_create_llm_client(
        ...
        """Resolve or create an LLM client for the agent."""
        provider, model = _parse_llm_identifier(manifest_llm_model or llm_config.get("llm") or "")
        ...
        if provider and model:
            ...
            cache_key = f"{agent_id}:{llm_config.get('model', 'default')}"

        if cache_key in self.agent_llm_clients:
            return self.agent_llm_clients[cache_key]

        # Prefer adapter registry factories when provider is known
        if self.adapter_registry and provider:
            client = self.adapter_registry.create_llm_client(provider, {**llm_config, "model": model})
            if client:
                self.agent_llm_clients[cache_key] = client
                return client

        # Fallback to default client (if provided)
        if self.llm_client:
            self.agent_llm_clients[cache_key] = self.llm_client
    ...
    def run_agent_stage(self, task: Task, node: Node, context_package) -> Tuple[Any | None, EngineError | None, Optional[Dict]]:
        ...
        if self.parameter_resolver and node.agent_id:
            try:
        ...
        # Get or create LLM client for this config (adapters or default)
        llm_client = self._get_or_create_llm_client(node.agent_id, llm_config, manifest_llm_model)
...
def _parse_llm_identifier(identifier: str) -> tuple[Optional[str], Optional[str]]:
    """Split provider/model strings like 'provider/model'."""
    if not identifier:
        return None, None
    if "/" in identifier:
